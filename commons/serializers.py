...
class DynamicFieldsModelSerializer(ModelSerializer):
    ...
    def __init__(self, instance=None, *args, **kwargs):
        ...
        super(DynamicFieldsModelSerializer, self).__init__(instance, *args, **kwargs)
    ...
    def get_fields(self):
        ...
        if has_current_user_model:
            for current_user_field in ["created_by", "updated_by"]:
                if current_user_field in fields.keys() and isinstance(
                    fields[current_user_field], serializers.PrimaryKeyRelatedField
                ):
                    fields[current_user_field] = serializers.UUIDField(
                        source=f"{current_user_field}.username",
                        format="hex",
                        read_only=True,
                    )
        return fields
